...
app.config["ALLOWED_FILES"] = {
    file_params[d]["file_name"] for d in file_params if "file_name" in file_params[d]
}
...
def process_excel_js(f_name):
    ...
    for file_type in file_params:
        if (
            "file_name" in file_params[file_type]
            and file_params[file_type]["file_name"] == f_name
        ):
            file_param = file_params[file_type]
    if len(file_param) != 0:
        process_file.generate_out(
            file_param, app.config["UPLOAD_FOLDER"], app.config["OUTPUT"]
        )
